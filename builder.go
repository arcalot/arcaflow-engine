...
func buildDependencyTree(inputSchema *schema.ScopeSchema, steps map[string]*workflow.PluginStep, stepSchemas map[string]schema.Step, output yaml.Node) (dgraph.DirectedGraph[treeItem], map[string]*workflow.PluginStep, any, error) {
    ...
    // We create dgraph nodes for all inputs.
    ...
    // We create dgraph nodes for all steps and their possible outputs.
    ...
    // We run through all the steps and compile the expressions in their deploy and input clauses to obtain dependencies.
    for stepID, step := range steps {
        ...
        // Need more? Add them here.

        steps[stepID] = step
    }
    // Finally, we evaluate output expressions.
...
func addDependencies(
    ...
    // See if the current YAML node is an ex pression
    ast, err := tryParseExpression(data)
    if err != nil {
        return nil, err
    }
    if ast == nil {
        // If not, process it as a normal YAML node and evaluate one level deeper.
        // YAML doesn't have data types, only strings, so this is fairly easy.
        switch data.Type() {
        case yaml.TypeIDMap:
    ...
    // Connect all the remaining possible items.
    for _, v := range items {
...
// buildExpressionSelector takes an AST node and builds a stack of expressions.
//nolint:funlen,gocognit
